...
const AnnouncementMessage = ({ announcement, canEdit, updateAnnouncement }) => {
  ...
  const deleteAnnouncement = async () => {
    try {
      announcement.files.forEach(async (file) => {
        await axios.delete(
          `${process.env.REACT_APP_BACKEND_URL}/api/uploads/delete/${file._id}?destination=announcements`,
          {
      ...
      await axios.delete(
        `${process.env.REACT_APP_BACKEND_URL}/api/announcement/delete/${announcement._id}`,
        {
          withCredentials: true,
        }
      );
  ...
  return (
    <>
      ...
      >
        האם הינך בטוח שברצונך למחוק את ההודעה{" "}
        <span style={{ fontWeight: 600 }}>{announcement.title}</span>, ההודעה
        תימחק לצמיתות!
      </Modal>
      {isEditing ? (
        ...
        <div className="announcement-container">
              ...
              <div className="announcement-top-end">
                <div className="announcement-info-field">
                  ...
                  <div className="announcement-author">
                    פורסם ע"י {announcement.writtenBy.name}
                  </div>
                </div>
                <div className="announcement-info-field">
                  <CalendarOutlined />
          ...
          <div
            ...
            dangerouslySetInnerHTML={{
              __html: processContent(announcement.content),
            }}
          />
          {announcement.files && (
            ...
            {announcement.updatedAt !== announcement.createdAt && (
              <div className="announcement-update-date">
                ...
                <div className="update-date-content">
                  עודכן לאחרונה ב{" "}
                  {formatDateWithoutSeconds(announcement.updatedAt)}
                </div>
              </div>
            )}
            {canEdit && (
