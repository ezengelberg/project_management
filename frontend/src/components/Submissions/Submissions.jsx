...
const Submissions = () => {
                                    ...
                                    <div
                                        ...
                                        <div className="table-col-info">
                                            {sub.fileNeeded ? (
                                                <Badge
                                                    color={
                                                        sub.submitted ? (sub.isLate ? "darkgreen" : "green") : "orange"
                                                    }
                                                    text={
                                                        sub.submitted
                                                            ? `הוגש${
                                                                  sub.isLate
                                                                      ? ` באיחור - ${Math.ceil(
                                                                            (new Date(sub.uploadDate) -
                                                                                new Date(sub.submissionDate)) /
                                                                                (1000 * 60 * 60 * 24),
                                                                        )} ימים`
                                                                      : ""
                                                              }`
                                            ...
                                            <div>
                                                {waitingCheck && (sub.submitted || !sub.fileNeeded) ? (
                                                  ...
                                                  ((sub.isGraded && sub.finalGrade === null) ||
                                                      (sub.isReviewed && sub.editable === true)) ? (
    ...
    const filteredSubmissionData = submissionData.filter((project) => {
        return (
            ...
            (selectedGroup === "all" ||
                groups.find((group) => group._id === selectedGroup)?.projects.includes(project.key))
        );
    });
        ...
        <div>
            <div className="action-buttons">
                ...
                <Button
                    ...
                    onClick={() => {
                        if (
                            projects.filter((project) => project.students.length !== 0 && project.advisors.length !== 0)
                                .length === 0
                        ) {
                            message.open({
                                type: "warning",
            ...
            <Modal
                ...
                <>
                    ...
                    <span style={{ fontWeight: "500" }}>
                        {deleteAllJudgesConfirm?.submissionName || "שם ברירת מחדל"}
                    </span>
                    {" מהשנה "}
            ...
            <Modal
                ...
                <Form layout="vertical" form={deleteJudgesForm}>
                    <p>
                        <span style={{ color: "red", fontWeight: 600 }}>שים לב</span> - האיפוס מוריד את כל השופטים עבור
                        ההגשות מהשנה{" "}
                    ...
                    <Form.Item
                        ...
                        <Select placeholder="בחר הגשה">
                            {submissionData
                                ...
                                .filter((sub) => sub.name)
                                .filter(
                                    (sub, index, array) => array.findIndex((item) => item.name === sub.name) === index,
                                )
                                .map((sub) => (
            ...
            <Modal
                ...
                <Form layout="vertical" form={judgeAssignmentForm}>
                    <p>
                        <span style={{ color: "red", fontWeight: 600 }}>שים לב</span> - ההקצאה תתבצע לכל ההגשות בשנה
                        הנבחרת{" "}
                        <Tooltip title="ניתן לשנות בחירה ב dropdown">
                            <span style={{ textDecoration: "underline" }}>{yearFilter}</span>
                        </Tooltip>
            ...
            <Modal
                ...
                <Form layout="vertical" form={deleteSubmissionsForm}>
                    <p>
                        <span style={{ color: "red", fontWeight: 600 }}>שים לב</span> - המחיקה מוחקת את כל ההגשות עם שם
                        זה עבור השנה{" "}
                        <Tooltip title="ניתן לשנות בחירה ב dropdown">
                            <span style={{ textDecoration: "underline" }}>{yearFilter}</span>
                        </Tooltip>
                    ...
                    <div className="footer-submission-actions">
                        <Tooltip title="מחיקת הגשה">
                            <DeleteOutlined
                                ...
                                onClick={() => setDeleteSubmission(submissionInfo)}
                            />
                        </Tooltip>
                        <Button
                            type="primary"
                                                      ...
                                                      submissionInfo.submission.isLate
                                                          ? ` באיחור - ${Math.ceil(
                                                                (new Date(submissionInfo.submission.uploadDate) -
                                                                    new Date(
                                                                        submissionInfo.submission.submissionDate,
                                                                    )) /
                                                                    (1000 * 60 * 60 * 24),
                                                            )} ימים`
                                                          : ""
                    ...
                    <>
                        ...
                        {showReview?.journalActive && (
                            <p>
                                ...
                                {showReview?.journalActive === "yes"
                                    ? "כן"
                                    : showReview?.journalActive === "no"
                                    ? "לא"
                                    : ""}
                            </p>
                        )}
            ...
            <Modal
                ...
                <Form layout="vertical" form={editSubmission}>
                    <p>
                        <span style={{ color: "red", fontWeight: 600 }}>שים לב</span> - העריכה משנה את כל ההגשות הזמינות
                        עם שם זה
                    </p>
                    <Form.Item
                        label="בחירת הגשה"
                        ...
                        <Select
                            ...
                            onChange={(value) => {
                                ...
                                const selectedSubmission = submissionDetails.find(
                ...
                <Form layout="vertical" form={formSpecific}>
                    ...
                    <Form.Item label="סוג הגשה" name="submissionType" initialValue={"proposalReport"} hasFeedback>
                        <Radio.Group
                            ...
                            options={submissionOptions}
                            onChange={(e) => {
                                const selectedName = submissionOptions.find(
                                    (option) => option.value === e.target.value,
                                ).value;
                                setSubmissionType(selectedName);
                                const selectedSubmission = submissionDetails.find(
                                    (submission) => submission.name === selectedName,
                                );
