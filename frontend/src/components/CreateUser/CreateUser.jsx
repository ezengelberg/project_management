    ...
    const handleSubmitCSV = async (users) => {
        try {
            const response = await axios.post(
                `${process.env.REACT_APP_BACKEND_URL}/api/user/register-multiple`,
                users,
                {
                    withCredentials: true,
                },
            ...
            if (response.data.existingUsers.length > 0) {
                message.warning(
                    `המשתמשים הבאים כבר קיימים במערכת: ${response.data.existingUsers
                        .map((user) => user.email)
                        .join(", ")}`,
                );
            }
            ...
            <div className="upload-users-csv">
                <Dragger className="uploader-users-csv" {...props}>
                    ...
                    <p className="ant-upload-hint">
                        יש להעלות קובץ CSV יחיד עם השורות הבאות: דוא"ל, שם פרטי, שם משפחה, ת"ז
                    </p>
                </Dragger>
                {users.length > 0 && (
