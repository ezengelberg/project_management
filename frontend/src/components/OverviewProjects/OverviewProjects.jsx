...
const OverviewProjects = () => {
        ...
        const fetchData = async () => {
            ...
            try {
                const [projectsRes, usersRes, submissionsRes] = await Promise.all([
                    axios.get(
                        `${process.env.REACT_APP_BACKEND_URL}/api/project/get-all-projects-by-year/${yearFilter}`,
                        {
                            withCredentials: true,
                        },
                    ),
                            ...
                            const candidates = await Promise.all(
                                project.candidates.map(async (candidate) => {
                                    ...
                                    return {
                                        ...
                                        status: project.students.some(
                                            (student) => student.student === candidate.student,
                                        ),
                                    };
                                }),
                            );
                ...
                const usersWithProject = new Set(
                    projectWithCandidates.flatMap((project) =>
                        project.students.map((student) => student.student.toString()),
                    ),
                );
                ...
                setUsersWithoutProjects(
                    activeUsers.filter(
                        (user) =>
                            !usersWithProject.has(user._id.toString()) &&
                            user.isStudent &&
                            !user.isAdvisor &&
                            !user.isCoordinator,
                    ),
    ...
    const handleDeleteProject = async () => {
        try {
            await axios.delete(
                `${process.env.REACT_APP_BACKEND_URL}/api/project/delete-project/${selectedProject._id}`,
                {
                    withCredentials: true,
                },
            );
    ...
    const candidatesExpandedRowRender = (record) => {
        const candidatesColumns = [
            ...
            },
            {
                ...
                key: "joinDate",
                render: (joinDate) =>
                    new Date(joinDate).toLocaleString("he-IL", { dateStyle: "short", timeStyle: "medium" }),
    ...
    const filteredTakenProjects = projects.filter((p) => {
        ...
        if (takenFilter === "missingJudges") {
            return (
                (p.alphaReportJudges?.length ?? 0) < 3 ||
                (p.finalReportJudges?.length ?? 0) < 3 ||
                (p.examJudges?.length ?? 0) < 3
            );
        }
                                  ...
                                  return advisorUser ? (
                                      <a
                                          ...
                                          <Highlighter
                                              ...
                                              autoEscape
                                              textToHighlight={
                                                  advisorUser.name.length > 20
                                                      ? `${advisorUser.name.substring(0, 20)}...`
                                                      : advisorUser.name
                                              }
                                          />
                                      </a>
                                      ...
                                      <React.Fragment key={`student-${index}`}>
                                          <a
                                              ...
                                              {studentUser.name.length > 20 ? (
                                                  <Tooltip title={studentUser.name}>
                                                      {studentUser.name.substring(0, 20)}...
                                                  </Tooltip>
                                              ) : (
                                                  studentUser.name
                                              )}
                            ...
                            <Button
                                onClick={() => {
                                    ...
                                    setUsersWithoutProjects((prev) => [
                                        ...prev,
                                        ...record.students
                                            .map((s) => users.find((u) => u._id === s.student))
                                            .filter(Boolean),
                                    ]);
                                }}>
                                עדכן סטודנטים
                                ...
                                <div key={submission._id} className="inner-table-order">
                                    {submission.isGraded && (
                                        <div className="show-grade">
                                            {`${
                                                submission.name.length > 18
                                                    ? submission.name.substring(0, 18) + "..."
                                                    : submission.name
                                            } - `}
                        ...
                        <Button
                            onClick={() => {
                                const projectSubmissions = submissions.filter(
                                    (submission) => submission.project === record._id,
                                );
        ...
        <div>
            ...
            <Modal
                ...
                onCancel={() => {
                    ...
                    setSelectedProject(null);
                    setUsersWithoutProjects(
                        usersWithoutProjects.filter((user) => !selectedStudents.includes(user._id)),
                    );
                }}
                    ...
                    <div key={submission._id} className="modal-select-input">
                        ...
                        <Select
                            ...
                            disabled={
                                ...
                                (!selectedSubmission?.extraData[index].isGraded &&
                                    !selectedSubmission?.extraData[index].isReviewed)
                            }
                        />
                    </div>
                ))}
